...
for (const captureMode of ['tagged', 'legacy', 'mixed']) {
  test(`maps ${captureMode} GTST captures through absolute numbers instead of provider seasons`, async () => {
    const tvdb = loadTvdb({
      seriesSeasons: [
    ...
    const legacyItem = { ...item, segment_type: 'outro' };
    delete legacyItem._tvdbAbsoluteTitleMatch;
    const capturedItems = captureMode === 'legacy' ? [legacyItem] : captureMode === 'mixed' ? [item, legacyItem] : [item];
    const result = await tvdb.mapSeriesItemsToTvdb(
      capturedItems,
    ...
    assert.deepEqual(plain(result.items.map(mapped => [mapped.season, mapped.episode])), capturedItems.map(() => [36, 180]));
