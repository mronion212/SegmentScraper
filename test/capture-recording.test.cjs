...
test('recapturing restored segments does not duplicate data or discard distinct movie credits', () => {
  ...
  const first = { _showId:'movie-one', _eid:'credits', media_type:'movie', segment_type:'outro', start_sec:100, end_sec:120, credit_part:'before_after_credits_scene' };
  state.allItems = [{ ...first, imdb_id:'tt123' }];
  recordExtractedSegments([{ ...first, imdb_id:'IMDB_PENDING' }, { ...first, _showId:'movie-two' }, { ...first, start_sec:130, end_sec:150, credit_part:'after_after_credits_scene' }]);
