...
test('capture recovery preserves movies, series mapping and IDs without persisting credentials', () => {
  const first = loadSession();
  first.restoreCaptureSession('skyshowtime');
  first.state.allItems = [{ _showId: 'movie', media_type: 'movie', imdb_id: 'tt123', start_sec: 10, end_sec: 20, credit_part: 'after_after_credits_scene' }];
  first.state.showIds.add('movie');
  ...
  assert.equal(restored.state.allItems[0].credit_part, 'after_after_credits_scene');
  assert.equal(restored.state.showIds.has('movie'), true);
