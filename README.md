...
Run `node --test` for regression tests and `node benchmark/serve-player-ui.cjs` for the local browser fixture at `http://127.0.0.1:8096`. The fixture runs 62 DOM checks across the five provider adapters, with controls for rerendering, waiting for controls, hidden controls, fullscreen, and a sample JSON preview. `http://127.0.0.1:8096/compact` embeds it in a 360 × 480 viewport. Prime Video, Videoland, and SkyShowtime fixtures reproduce the supplied control nesting with simulated native styles. Additional cases cover a fixed-width Netflix fullscreen wrapper, provider CSS that recolors SVG paths, a 28px isolated white logo, and overlap at narrow/wide player widths. Checks also cover hidden controls and rerendering; they are not live provider compatibility tests.
