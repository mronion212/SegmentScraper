...
let activeRun, uploadSnapshot='', jobOptions='', editorDirty=false, analysisSnapshot='', requestedAnalysisJob=null;
function invalidateUpload(){activeRun=null;editorDirty=true;$('review-confirm').checked=false;$('submit-upload').disabled=true;$('upload-summary').textContent='Changes need a new validation run';}
...
function loadUploadJob(){
 const job=jobs.find(j=>j.id===$('upload-job').value);$('segment-editor').replaceChildren();invalidateUpload();if(!job?.report)return;
...
bind('analyze-ending',async()=>{
 const jobId=$('upload-job').value;if(!jobId)throw new Error('Choose an inspected file first.');
 await api('analyze-credits',{jobId,scanFraction:Number($('analysis-window').value)});requestedAnalysisJob=jobId;analysisSnapshot='';invalidateUpload();$('ending-reviewed').checked=false;await poll();
});
...
function renderEndingAnalysis(job){
 ...
 const analysis=job?.report?.analysis;const snapshot=JSON.stringify([job?.id,analysis,job?.previews]);if(snapshot===analysisSnapshot)return;analysisSnapshot=snapshot;
 ...
 analysis.scenes.forEach((scene,index)=>{
   ...
   for(const [value,text]of [['','Choose after review'],['scene','Real scene — preserve it'],['not-scene','False positive — credits/logo/other']]){const opt=el('option',text);opt.value=value;select.append(opt);}select.onchange=()=>{invalidateUpload();$('ending-reviewed').checked=false;};label.append(select);$('analysis-findings').append(label);
 });
 for(const clip of job.previews||[]){
   const detail=el('details',undefined,'preview-card'),summary=el('summary',clip.kind==='overview'?`Ending overview · 12× speed · ${time(clip.sourceStart)}–${time(clip.sourceEnd)}`:`Boundary at ${time(clip.boundary)} · 12-second context`),video=el('video');video.controls=true;video.preload='none';video.src=clip.url;video.setAttribute('aria-label',summary.textContent);const stamp=el('small',`Source starts at ${time(clip.sourceStart)}`);video.ontimeupdate=()=>{stamp.textContent=`Source time: ${time(clip.sourceStart+video.currentTime*clip.speed)}`;};detail.append(summary,video,stamp);$('analysis-previews').append(detail);
 }
