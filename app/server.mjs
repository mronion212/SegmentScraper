...
import { analyzeCredits, disposeAnalysis } from './analysis.mjs';

const publicDir = fileURLToPath(new URL('./public/', import.meta.url));
const terminal = new Set(['done', 'error', 'cancelled']);
export function createApp({ downloadDir = path.resolve('app-data/downloads'), probe = inspectFile, remoteProbe = inspectRemote, credentialStore, providerFactory = (p, t) => new Debrid(p, t), downloader = downloadFile, uploads = createUploadService(), updates, analyzer=analyzeCredits } = {}) {
  ...
  const view = job => ({ id: job.id, name: job.name, status: job.status, remote: job.remote === true, bytes: job.bytes, total: job.total, error: job.error, report: job.report, savedPath: job.savedPath, analysisProgress:job.analysisProgress, analysisError:job.analysisError, previews:(job.analysisResult?.artifacts||[]).map(({file,...a})=>({...a,url:`/preview/${a.id}`})) });
  async function work() {
    if (running) return;
    running = true;
    try {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if(req.method==='GET'&&url.pathname.startsWith('/preview/')){
        ...
        if(req.headers.range){const match=req.headers.range.match(/^bytes=(\d+)-(\d*)$/);if(!match)return send(res,416,{error:'Invalid range'});start=Number(match[1]);end=match[2]?Number(match[2]):end;if(start>end||end>=size){res.setHeader('Content-Range',`bytes */${size}`);return send(res,416,{error:'Invalid range'});}}
        res.writeHead(req.headers.range?206:200,{'Content-Type':'video/mp4','Content-Length':end-start+1,'Accept-Ranges':'bytes','Cache-Control':'no-store',...(req.headers.range?{'Content-Range':`bytes ${start}-${end}/${size}`}:{})});
      ...
      if (!url.pathname.startsWith('/api/')) {
        const assets = { '/': ['index.html', 'text/html'], '/style.css': ['style.css', 'text/css'], '/app.js': ['app.js', 'text/javascript'], '/upload.js': ['upload.js', 'text/javascript'] };
      ...
      if (req.method === 'GET' && url.pathname === '/api/status') return send(res, 200, { jobs: jobs.map(view), providers: [...connections.keys()], downloadDir, uploads: uploads.list(), adminConfigured: uploads.adminConfigured(), update: updates?.get() });
      ...
      if(url.pathname==='/api/analyze-credits'){
        ...
        if(jobs.some(j=>j.status==='analyzing'))throw new Error('An ending analysis is already running.');
        ...
        void(async()=>{
          try{
            const source=job.remote?{remote:true,input:await job.analysisProvider.downloadLink(job.torrentId,job.fileId,controller.signal)}:{remote:false,input:job.local||job.savedPath};
            const result=await analyzer(source,job.report,{signal:controller.signal,scanFraction:body.scanFraction??.25,onProgress:p=>{if(job.controller===controller)job.analysisProgress={...job.analysisProgress,...p,updatedAt:Date.now()};}});
      ...
      if (url.pathname === '/api/submit-upload') return send(res, 202, await uploads.submit(body.runId,body));
      ...
      if (url.pathname === '/api/clear') {
        if(uploads.active())throw new Error('Wait for validation or upload to finish before clearing reports.');
  ...
  server.hasActiveJobs = () => uploads.active() || jobs.some(job => !terminal.has(job.status));
  ...
  server.stopJobs = () => { for (const j of jobs) {if (!terminal.has(j.status)) { j.controller.abort(); j.status = 'cancelled'; }void disposeAnalysis(j.analysisResult);} connections.clear(); };
